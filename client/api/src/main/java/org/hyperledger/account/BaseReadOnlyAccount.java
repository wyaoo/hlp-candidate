...
public class BaseReadOnlyAccount implements ReadOnlyAccount {
    ...
    private TransactionOutput removeOutput(Outpoint p) {
        ...
        if (out != null) {
            log.trace("Remove " + p + " (" + out.getOutputAddress() + ") " + out.getValue());
        }
        return out;
    }
    ...
    private boolean processInputs(Transaction t) {
        ...
        for (TransactionInput input : t.getInputs()) {
            ...
            spend = confirmed.remove(new Outpoint(input.getSourceTransactionID(), input.getOutputIndex()));
            if (spend != null) {
                log.trace("Spend settled output " + input.getSourceTransactionID() + " [" + input.getOutputIndex() + "] " + spend.getValue());
            } else {
                spend = change.remove(new Outpoint(input.getSourceTransactionID(), input.getOutputIndex()));
                if (spend != null) {
                    log.trace("Spend change output " + input.getSourceTransactionID() + " [" + input.getOutputIndex() + "] " + spend.getValue());
                } else {
                    spend = receiving.remove(new Outpoint(input.getSourceTransactionID(), input.getOutputIndex()));
                    if (spend != null) {
    ...
    private boolean processMempoolOutputs(APITransaction t, boolean spending) {
        ...
        for (Coin coin : t.getCoins()) {
            ...
            if (addressChain.getRelevantAddresses().contains(o.getOutputAddress())) {
                if (spending) {
                    change.add(coin);
                    log.trace("Change " + t.getID() + " [" + p.getOutputIndex() + "] (" + o.getOutputAddress() + ") "
                            + o.getValue());
                } else {
                    modified |= receiving.add(coin);
                    log.trace("Receiving " + t.getID() + " [" + p.getOutputIndex() + "] (" + o.getOutputAddress() + ") "
                            + o.getValue());
                }
            }
        }
        ...
        synchronized (this) {
            if (command.equals("tx") && transactions.containsKey(new TID(hash))) {
                ...
                for (TransactionInput input : revert.getInputs()) {
                    ...
                    if (spend) {
                        change.add(prev.getCoin(input.getOutputIndex()));
                    } else {
                        receiving.add(prev.getCoin(input.getOutputIndex()));
                    }
                }
    ...
    public synchronized void unconfirmed(APITransaction t) {
        for (Coin p : t.getCoins()) {
            confirmed.remove(p.getOutpoint());
        }
